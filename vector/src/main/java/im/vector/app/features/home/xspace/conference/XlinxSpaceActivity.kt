...
class XlinxSpaceActivity : VectorBaseActivity(), JitsiMeetActivityInterface, JitsiMeetViewListener {
    ...
    private fun configureJitsiView(viewState: JitsiCallViewState) {
        val serverURL = tryOrNull { URL("https://x-linx.space") }
        ...
        Log.i("jitsi", viewState.roomId + viewState.nickName)
    }
...
//    }
    ...
    companion object {
        fun newIntent(context: Context, roomId: String, nickName: String, enableVideo: Boolean): Intent {
            return Intent(context, XlinxSpaceActivity::class.java).apply {
                putExtra(MvRx.KEY_ARG, Args(roomId, nickName, enableVideo))
